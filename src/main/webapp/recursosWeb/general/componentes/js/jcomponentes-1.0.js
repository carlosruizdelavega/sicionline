...
$(document).ready(function(){
	...
	//inicializar reconocimiento de peticiones ajax
	//fnSettingAjaxJquery();
	
});

/**
 * funcion que permite cargar contenido html en div, a traves de una peticion ajax
 */
function fnCargarContenido(url, param, idDivHtml){
		...
		$("body")
				.append(
						'<div id="cLoader" class="ui-widget-overlay ui-front"></div>'
								...
								+ '<img src="recursosWeb/images/imgLoader.gif" border="0" />'
								+ '</div>' + '</div>');
...
function fnSettingAjaxJquery(){
  ...
  jQuery(document).ajaxError(function(event, XMLHttpRequest, ajaxOptions, thrownError){
     ...
     if(XMLHttpRequest.responseText=='-1'){
    	 ...
    	 alert("Su sesion ha expirado.","onSesionExpirada()");
    	 //alerta("Su sesion ha expirado.","onSesionExpirada()");        
      return;
     }
     
     alert(XMLHttpRequest.responseText+"<br/><br/>(status error:"+XMLHttpRequest.status+")");
...
function fnMostrarModalCargando(){
    ...
    html += "<p>Espere un momento por favor...  <img alt='Cargando' src='recursosWeb/images/imgLoader.gif'/><\/p>";
    html += "<\/div>";
        $('body').append($(html));
